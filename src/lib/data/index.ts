...
function preparePosts(posts: SocialPost[]): SocialPost[] {
  return enrichEstimatedClicks(enrichPostsWithStoryBeat(posts));
}
...
export async function getLinkedInPosts(): Promise<{
  ...
  if (cache?.posts?.length) {
    return {
      posts: preparePosts(cache.posts),
      source: "cache",
      meta: {
  ...
  return { posts: preparePosts(allPosts), source: "mock" };
}
